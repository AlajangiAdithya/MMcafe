...
export default function VaporizeTextCycle({
  ...
  const vaporizeEndFiredRef = useRef(false);
  ...
  const onVaporizeEndRef = useRef(onVaporizeEnd);
  onVaporizeEndRef.current = onVaporizeEnd;
  const onCycleCompleteRef = useRef(onCycleComplete);
  onCycleCompleteRef.current = onCycleComplete;
  const updateFnRef = useRef(memoizedUpdateParticles);
  updateFnRef.current = memoizedUpdateParticles;
  const renderFnRef = useRef(memoizedRenderParticles);
  renderFnRef.current = memoizedRenderParticles;
  const durationsRef = useRef(animationDurations);
  durationsRef.current = animationDurations;
  ...
  useEffect(() => {
    ...
    } else if (!isInView && !hasStartedRef.current) {
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      switch (state) {
        case "static": {
          renderFnRef.current(ctx, particlesRef.current);
          break;
        }
        case "vaporizing": {
          ...
          const allVaporized = updateFnRef.current(particlesRef.current, vaporizeX, deltaTime);
          renderFnRef.current(ctx, particlesRef.current);

          if (vaporizeProgressRef.current >= 100 && allVaporized) {
            if (onVaporizeEndRef.current && !vaporizeEndFiredRef.current) {
              vaporizeEndFiredRef.current = true;
              onVaporizeEndRef.current();
            }
...
const parseColor = (color: string) => {
  ...
  if (rgbMatch) { const [_, r, g, b] = rgbMatch; return `rgba(${r}, ${g}, ${b}, 1)`; }
  return "rgba(0, 0, 0, 1)";
};
...
export { VaporizeTextCycle as VaporizeText };
